...
public class Cam1 extends CameraController {
    ...
    public void update(float delta) {
        ...
        camera.position.lerp(newPosition.add(direction.scl(2f)), Math.abs(car.getSpeed()) * delta * 0.5f); //TODO: rework tracking
        camera.position.y = MathUtils.lerp(camera.position.y, camera.position.y + 1f, Math.abs(car.getSpeed()) * delta * 0.5f);
