...
public class BasicBulletScreen extends BaseScreen {
    /**
    ...
    public BasicBulletScreen(Game game) {
        super(game);
        ...
        ModelBuilder modelBuilder = new ModelBuilder();
        modelBuilder.begin();
        Material material = new Material(ColorAttribute.createDiffuse(getRandomColor()));
        MeshPartBuilder builder = modelBuilder.part("box", GL20.GL_TRIANGLES, VertexAttributes.Usage.Position | VertexAttributes.Usage.Normal, material);

        BoxShapeBuilder.build(builder, 0, 0, 0, 1f, 1f, 1f);

        Model model = modelBuilder.end();
        GameObject gameObject = new GameObject(model, new btBoxShape(new Vector3(0.5f, 0.5f, 0.5f)));

        gameObject.transform.setToTranslation(0, 10, 0);
    ...
    private void createFloor() {
        ...
        floorObject.body.setWorldTransform(floorObject.transform); //! Bounding Box muss immer mit verschoben werden
